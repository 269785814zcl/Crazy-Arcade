...
def select_character_move1 (screen,num):  #选择人物时的动画1，用于图片上出现对勾
	if num == 1:
		screen.blit (center,(190,145))
	elif num == 2:
		screen.blit (center,(350,60))
	elif num == 3:
		screen.blit (center,(570,170))
	elif num == 4:
		screen.blit (center,(750,100))
...
def main_select_character (screen,number_x,unit,h,l):
	...
	while True:  #人物选择循环
		...
		if number == 0:
			...
			rolex=[]
			roley=[]
			rolex.append([100.,100.,140.,140.])      #角色初始横坐标位置
			roley.append([100.,140.,100.,140.]) 	 #角色初始纵坐标位置
			rolex.append([1060.,1060.,1100.,1100.])  #角色初始横坐标位置
			roley.append( [100.,140.,100.,140.]) 	 #角色初始纵坐标位置
			rolex.append( [100.,100.,140.,140.]) 	 #角色初始横坐标位置
			roley.append( [660.,700.,660.,700.]) 	 #角色初始纵坐标位置
			rolex.append( [1060.,1060.,1100.,1100.]) #角色初始横坐标位置
			roley.append( [660.,700.,660.,700.]) 	 #角色初始纵坐标位置
			for i in range (0,number_x ):
				role = Role (rolex[i],roley[i],6,array[i][1],3,2,unit,h,l)
